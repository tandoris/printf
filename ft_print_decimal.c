...
/*   Updated: 2019/08/20 04:28:21 by clboutry         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_print_right2(intmax_t nbr, int neg, t_struct *info)
{
	...
	if (info->space && neg == 0 && info->width == 0)
		write(1, " ", 1);
...
void		ft_print_right(intmax_t nbr, t_struct *info)
{
	...
	if (ft_print_right2(nbr, neg, info))
		return ;
	if (info->plus && neg == 0 && info->width && info->precision_find == 0)
		nbrlen++;
	...
	info->precision = (info->width > info->precision) ? info->width : info->precision;
